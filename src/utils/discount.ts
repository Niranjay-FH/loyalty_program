...
export function canRedeemPoints(
  ...
  // Get customer's loyalty info for this store
  const loyaltyInfo = customer.loyaltyInfo.find(
    info => info.storeId === store.storeId
  );

  if (!loyaltyInfo) {
    return { 
      canRedeem: false, 
      reason: 'Customer not enrolled in this store loyalty program' 
    };
  }
...
export function canRedeemPointsSimple(
  customer: Customer, 
  basket: BasketResponse, 
  store: Store
): boolean {
  return canRedeemPoints(customer, basket, store).canRedeem;
}

export function arePointsValid(customer: Customer, store: Store): boolean {
  const maxExpiryDays = store.loyaltyPartner.validationRules?.maxPointsExpiryDays;
  
  if (!maxExpiryDays) {
    return true; // No expiry rule
  }
  ...
  const joinedDate = new Date(loyaltyInfo.membership.joinedDate);
  const now = new Date();
  const daysSinceJoined = Math.floor(
    (now.getTime() - joinedDate.getTime()) / (1000 * 60 * 60 * 24)
  );

  return daysSinceJoined <= maxExpiryDays;
}
